...
console.info("[YouTube Tab Time Counter] Background worker loaded.");
...
const getDurationForTab = async (tab) => {
  ...
  if (tab?.url) {
    ...
    if (contentScriptFailed && isVideoUrl(tab.url)) {
...
const refreshTotals = async () => {
  console.info("[YouTube Tab Time Counter] Refreshing totals...");
  await loadDurationCache();
  ...
  console.info("[YouTube Tab Time Counter] Totals updated.", {
    totalTabs: tabs.length,
    knownCount,
    unknownCount,
    totalSeconds,
...
browser.runtime.onInstalled.addListener(() => {
  console.info("[YouTube Tab Time Counter] onInstalled fired.");
  refreshTotals();
});

browser.runtime.onStartup.addListener(() => {
  console.info("[YouTube Tab Time Counter] onStartup fired.");
  refreshTotals();
});
...
browser.runtime.onMessage.addListener((message) => {
  if (message?.type === "refreshTotals") {
    console.info("[YouTube Tab Time Counter] refreshTotals message received.");
